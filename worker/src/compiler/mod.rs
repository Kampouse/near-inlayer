...
// mod docker; // removed for standalone
mod native; // Native compilation with bubblewrap (for TEE/Phala)

/// Compilation error with user-friendly message
...
impl std::fmt::Debug for CompilationError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "Compilation failed: {} (exit: {:?})", self.user_message, self.exit_code)
    }
}
...
impl Compiler {
    ...
    async fn compile_from_github(&self, repo: &str, commit: &str, build_target: &str) -> Result<Vec<u8>> {
        ...
        native::compile(repo, commit, &normalized_target, Some(self.config.compile_timeout_seconds)).await
    }
