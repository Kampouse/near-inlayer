...
// ════════════════════════════════════════════════════════════════════════
// Worker: Post Result (kind 41002)
// ════════════════════════════════════════════════════════════════════════

/// Worker CLI subcommand — signs claim + submit_result with worker's msig key,
/// posts kind 41002 to Nostr with the result and signed actions.
///
/// Usage:
///   inlayer post-result \
///     --nostr-key <nsec_hex> \
///     --worker-key <ed25519_hex_or_base58> \
///     --msig <worker_msig_account> \
///     --escrow <escrow_contract> \
///     --job-id <job_id> \
///     --result <result_text_or_file> \
///     --kv-account <kv_account> \
///     --stake <yocto> \
///     --relay <wss://relay_url>
pub fn cmd_post_result(args: &[String], _config_dir: &Path) -> Result<()> {
    let mut rpc_url = "https://rpc.testnet.near.org".to_string();
    let mut relay = "wss://nostr-relay-production.up.railway.app/".to_string();
    let mut nostr_key = String::new();
    ...
    let mut kv_account = String::new();
    let mut stake = "100000000000000000000000".to_string(); // 0.1 NEAR default
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--relay" | "--relays" => { relay = args[i + 1].clone(); i += 2; }
            "--nostr-key" => { nostr_key = args[i + 1].clone(); i += 2; }
            "--worker-key" => { worker_key = args[i + 1].clone(); i += 2; }
            "--msig" => { msig = args[i + 1].clone(); i += 2; }
            "--escrow" => { escrow = args[i + 1].clone(); i += 2; }
            "--job-id" => { job_id = args[i + 1].clone(); i += 2; }
            "--result" => { result = args[i + 1].clone(); i += 2; }
            "--result-file" => { result_file = args[i + 1].clone(); i += 2; }
            "--kv-account" => { kv_account = args[i + 1].clone(); i += 2; }
            "--stake" => { stake = args[i + 1].clone(); i += 2; }
            _ => { eprintln!("Unknown arg: {}", args[i]); i += 1; }
        }
    }

    if nostr_key.is_empty() || worker_key.is_empty() || msig.is_empty()
        || job_id.is_empty() || (result.is_empty() && result_file.is_empty())
    {
        bail!("Required: --nostr-key, --worker-key, --msig, --job-id, --result (or --result-file)");
    }

    // Load result from file if specified
    let result_output = if !result_file.is_empty() {
        std::fs::read_to_string(&result_file)
            .context(format!("read result file: {}", result_file))?
    } else {
        result.clone()
    };
    ...
    let rpc = make_rpc(&rpc_url)?;
    let current_nonce = get_msig_nonce(&rpc, &msig).unwrap_or(0);
    let claim_nonce = current_nonce + 1;
    let submit_nonce = claim_nonce + 1;

    // 2. Build + sign claim() action
    //    kv_key is deterministic: result/{job_id}
    let kv_key = format!("result/{}", job_id);
    let stake_yocto: u128 = stake.parse::<u128>().unwrap_or(100_000_000_000_000_000_000_000u128);

    let claim_action = serde_json::json!({
        "nonce": claim_nonce,
        "action": {
            "type": "call",
            "receiver_id": escrow,
            ...
            "deposit": format!("{}", stake_yocto),
        }
    });
    let claim_action_json = serde_json::to_string(&claim_action).context("serialize claim action")?;
    let claim_sig = sign_action_ed25519(&claim_action_json, &worker_key)?;
    let claim_sig_hex = hex::encode(&claim_sig);

    println!("Claim action signed: nonce={}, {} byte sig", claim_nonce, claim_sig.len());

    // 3. Build + sign submit_result() action
    //    The kv_reference is deterministic — worker knows it at sign time
    let kv_ref_json = serde_json::json!({
        "kv_account": if kv_account.is_empty() { "kv.fastnear.near" } else { &kv_account },
        "kv_predecessor": msig,
        "kv_key": kv_key,
    });

    let submit_action = serde_json::json!({
        "nonce": submit_nonce,
        "action": {
            "type": "call",
            "receiver_id": escrow,
            ...
            "args": {
                "job_id": job_id,
                "result": serde_json::to_string(&kv_ref_json)?,
            },
            "gas": 100_000_000_000_000u64,
            "deposit": "0",
        }
    });
    let submit_action_json = serde_json::to_string(&submit_action).context("serialize submit action")?;
    let submit_sig = sign_action_ed25519(&submit_action_json, &worker_key)?;
    let submit_sig_hex = hex::encode(&submit_sig);

    println!("Submit action signed: nonce={}, {} byte sig", submit_nonce, submit_sig.len());

    // 4. Post kind 41002 to Nostr
    let content = serde_json::json!({
        "job_id": job_id,
        "output": result_output,
    });
    let content_str = serde_json::to_string(&content).context("serialize content")?;

    let tags = vec![
        vec!["job_id".into(), job_id.clone()],
        vec!["worker_msig".into(), msig.clone()],
        vec!["claim_action".into(), claim_action_json],
        vec!["claim_sig".into(), claim_sig_hex],
        vec!["submit_action".into(), submit_action_json],
        vec!["submit_sig".into(), submit_sig_hex],
    ];

    let relay = relay.trim_end_matches('/');
    super::nostr::publish_event(
        relay,
        &nostr_key,
        super::nostr::KIND_RESULT,
        &content_str,
        tags,
    )
    .map_err(|e| anyhow::anyhow!("publish: {}", e))?;

    println!("Result posted: job_id={}", job_id);
    println!("  kind=41002, claim_nonce={}, submit_nonce={}", claim_nonce, submit_nonce);
    println!("  kv_reference={}", serde_json::to_string(&kv_ref_json)?);
    println!("  worker_msig={}", msig);
    Ok(())
}
